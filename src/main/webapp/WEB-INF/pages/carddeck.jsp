...
<span class="playstyle">
	...
	<select name="suit" ng-disabled="!aceSelected" ng-model="selectedItem" ng-options="key as value for (key, value) in suit"></select>
...
function($scope, $http, $timeout, CARD_SUITES) {
     ...
     (function tick() {
         ...
         $scope.setSelected = function(card) {
             ...
             $scope.selectedItem = card.suit;
             
             console.log("Select aceSelected:\t" + card.ace); 
             console.log("Select suit :\t" + card.suit); 
         }
         ...
         $scope.playCard = function() {
        	 ...
        	 if (angular.isUndefined($scope.selected)) {
        	 	...
        	 	var aceSuit = $scope.aceSelected? $scope.selectedItem: null;
        	 	var playerObj = { playerName: "${playerName}", cardId: $scope.selected.id, aceSuit: aceSuit };
        	 	
        	 	$http.post('/playCard', playerObj) 
         ...
         $http.get('/getPlayerCards', { params: { playerName: "${playerName}" }}).success(function (response) {
			  ...
			  $scope.aceSuit = response.aceSuit !== null? CARD_SUITES[response.aceSuit]: null;
